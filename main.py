...
class Board:
    ...
    def fire(self, p):
        ...
        for ship in self.ships: # цикл проверки попадания
            if p in ship.points: # если попали...
...
class Player: # класс с общими атрибутами и методами для классов (игроков) User и AI (artificial intelligence)
    ...
    def move(self):
        while True: # создаем бесконечный цикл в котором
            ...
            except BoardException as e: # если огонь нанесен неверно, то обрабатываем исключение и продолжаем выполнение цикла
...
class Game:
    def __init__(self, size = 6): # конструктор в котором задаем размер поля, и генерируем размещение кораблей как для User, так и для AI
        self.size = size
        pl = self.random_board()
        co = self.random_board()
        co.hid = True # скрываем игровое поле AI

        self.ai = AI(co, pl) # создаем игрока AI
        self.us = User(pl, co) # создаем игрока User
        # передаем им создаваемые поля:
    ...
    def greetings(self): # окно приветствия
        print("-------------------")
    ...
    def play(self):
        num = 0 # счетчик ходов (номеров ходов)
        while True:
            print('-' * 20)
            print('ДОСКА ИГРОКА:')
            print(self.us.board) # отображение доски созданной для пользователя
            print('-' * 20)
            print('ДОСКА КОМПЬЮТЕРА:')
            print(self.ai.board)
            if num % 2 == 0: # если номер ход четный, то ходит пользователь, иначе...
                print('-' * 20)
                print('Ходит игрок!')
                repeat = self.us.move() # вызов метода move (хода), с записью результата в repeat (см. метод move в классе Player)
            else:
                print('-' * 20) # ходит компьютер
                print('Ходит компьютер!')
                repeat = self.ai.move() # вызов метода move (хода), с записью результата в repeat (см. метод move в классе Player)
            if repeat: # в случае если выполняется repeat, то ход не "засчитываем", для того чтобы ход остался у того кто сейчас "удачно" сходил
                num -= 1
            if self.ai.board.count == 7: # проверка условия равенства количества потопленных кораблей с общим количеством в списке lens
                print('-' * 20)
                print('Игрок победил!')
                break
            if self.us.board.count == 7:
                ...
                break
            num += 1 # если ход завершен, то увеличиеваем счетчик ходов num на единицу

    def start(self): # метод который включает в себя приветствие и игровой процесс
        self.greetings()
        self.play()

g = Game() # создание экземпляра g класса Game
g.start() # применение метода start для g (запуск игры). Удачи!